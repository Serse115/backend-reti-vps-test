...
public class DBDeliveryOperations implements DBDeliveryOperationsInterface {
    ...
    // Method to retrieve the full delivery order codes and their info
    ...
    public String[] getTheDeliveryOrderCodes() {
        ...
        try {
            ...
            // Getting the data
            while (rs.next()) {
                data.add(rs.getString("order_code"));       // 2. Add the string to the list
            }

            // Then convert the list to an array of strings
    ...
    public boolean insertDeliveryOrderList(String orderCode, List<Object[]> items) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            conn.setAutoCommit(false);                                  // Deactivating auto-commit to handle everything as a single operation (transaction)
            ...
            pstmt.executeBatch();                       // Execute by sending everything to the db
            conn.commit();                              // Confirm the operation
