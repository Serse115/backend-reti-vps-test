...
public class OrderController {
    ...
    // Fetch the credentials from the properties file
    ...
    @Autowired
    private DataSource dataSource;  // Automatically handle the connection pool of HikariCP connections

    // Waiting room for orders waiting to be confirmed
    ...
    public ResponseEntity<?> requestOtp(@RequestBody OrderRequestDTO request) {
        ...
        try (Connection conn = this.dataSource.getConnection()) {
            ...
            if (otpService.saveOTP(request.getEmail(), code)) {
                ...
                // Park the order in memory waiting for the confirmation
    ...
    public ResponseEntity<?> verifyOtpAndSubmit(@RequestParam("email") String email, @RequestParam("otp") String otp) {
        ...
        try (Connection conn = this.dataSource.getConnection()) {
            ...
            // Verify the otp and delete it if it's correct
            if (otpService.verifyOTP(email, otp)) {

                // Retrieves the parked down order
                OrderRequestDTO originalRequest = this.pendingOrders.get(email);

                if (originalRequest != null) {
                    // Saves the order and executes the final submit
                    String orderCode = this.executeFinalSubmit(originalRequest);

                    if (orderCode != null) {
                        this.pendingOrders.remove(email);      // Clean the saved order from the waiting room
    ...
    public ResponseEntity<List<OrderRequestDTO>> getDeliveryOrdersFullList() {
        ...
        // Initialize the connection through the data source
        try (Connection conn = this.dataSource.getConnection()) {
    ...
    private String executeFinalSubmit(OrderRequestDTO request) {
        ...
        try (Connection conn = this.dataSource.getConnection()) {
            ...
            if (infoSaved) {
                // Converting the DTO in the correct format
    ...
    // Counting the orders method (with 5 secs timer)
    ...
    // Method to get the delivery orders summary (order code - status)
    ...
    // Method to get the details of a single chosen order
    ...
    // Method to update the status of a delivery order (awaiting - ongoing - ready)
    ...
    public ResponseEntity<List<DeliveryOrdersFullListDTO>> getAllDeliveryCardsData() {
        ...
        try (Connection conn = this.dataSource.getConnection()) {
            ...
            // Retrieve the list of order codes and statuses
            ...
            if (orders != null) {
                for (String[] order : orders) {
                    ...
                    // Get the delivery order data for each order code
    ...
    // Endpoint method to delete a delivery order
