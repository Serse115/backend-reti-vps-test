...
// Method to request the OTP
    ...
    public ResponseEntity<?> requestOtp(@RequestBody OrderRequestDTO request) {
        ...
        try (Connection conn = this.dataSource.getConnection()) {
            ...
            // Clear all the previously expired OTPs
            otpService.cleanExpiredOTPs();

            // 1. Genera l'OTP
            String code = otpService.generateOTP();

            // 2. Salva l'OTP nel DB
            if (otpService.saveOTP(request.getEmail(), code)) {
                ...
                try {
                    ...
                    System.out.println(
                            ">>> OTP inviato a " + request.getEmail() + ": " + code
                    );

                    return ResponseEntity.ok(
                            "OTP inviato all'email: " + request.getEmail()
                    );

                } catch (Exception e) {
                    e.printStackTrace();
                    return ResponseEntity.status(500)
                            .body("Errore nell'invio dell'email.");
                }
            }

            return ResponseEntity.status(500)
                    .body("Errore nella generazione dell'OTP.");

        } catch (SQLException e) {
            System.err.println(
                    "Errore DB full-cards-data: " + e.getMessage()
            );
    ...
    public ResponseEntity<?> verifyOtpAndSubmit(
            ...
            if (otpService.verifyOTP(email, otp)) {
                ...
                if (originalRequest != null) {
                    ...
                    if (orderCode != null) {
                        ...
                        // Pulizia
                        this.pendingOrders.remove(email);

                        return ResponseEntity.ok(orderCode);
                    }

                    return ResponseEntity.status(500)
                            .body("Errore durante il salvataggio dell'ordine nel database.");
                }

                return ResponseEntity.status(400)
                        .body("Sessione ordine scaduta. Ricomincia il processo.");
            }

            return ResponseEntity.status(401)
                    .body("Codice OTP errato o scaduto.");

        } catch (SQLException e) {

            return ResponseEntity.status(503)
                    .body("Database non raggiungibile.");
        }
    }
    ...
    public ResponseEntity<?> sendOrderResume(
        ...
        try (Connection conn = this.dataSource.getConnection()) {
            ...
            try {
                ...
                System.out.println(
                        ">>> Email di conferma e riepilogo inviata a "
                                + request.getEmail()
                );

                return ResponseEntity.ok(
                        "Riepilogo di conferma inviato all'email: "
                                + request.getEmail()
                );
...
// Method to retrieve the menu data from the DB
    @GetMapping("/menu")
    public ResponseEntity<?> getPublicMenu() {
        ...
        try (Connection conn = this.dataSource.getConnection();
            ...
            System.err.println(
                    "Errore query menu: " + e.getMessage()
            );
    ...
    // Method to retrieve the delivery orders list from the DB
    @GetMapping("/full-delivery-orders")
    public ResponseEntity<List<OrderRequestDTO>> getDeliveryOrdersFullList() {

        try (Connection conn = this.dataSource.getConnection()) {

            DBDeliveryOperationsInterface dbDelivery =
                    new DBDeliveryOperations(conn);

            List<OrderRequestDTO> orderRequest =
                    dbDelivery.getTheDeliveryOrderCodesAndTheirInfo(1);
...
// Metodo helper privato che contiene la tua vecchia logica di salvataggio
    private String executeFinalSubmit(OrderRequestDTO request) {
        ...
        try (Connection conn = this.dataSource.getConnection()) {
            ...
            // 1. Salviamo l'anagrafica usando i getter del DTO
            ...
            if (infoSaved) {
                ...
                // 2. Convertiamo la lista di DTO nel formato richiesto
                ...
                if (dbDelivery.insertDeliveryOrderList(
                        orderCode,
                        itemsForDb)) {
    ...
    // Sub method to create the order code
    private String createTheOrderCode() {
        ...
        final String charactersToUse =
                "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";

        StringBuilder sb =
                new StringBuilder(8);

        // Loop to generate 8 characters
        for (int i = 0; i < 8; i++) {

            sb.append(
                    charactersToUse.charAt(
                            random.nextInt(
                                    charactersToUse.length()
                            )
                    )
            );
        }
    ...
    public ResponseEntity<?> getTodayOrders() {
        ...
        try (Connection conn = this.dataSource.getConnection();
             ...
             ResultSet rs = stmt.executeQuery(
                     "SELECT meal_code FROM order_management")) {
            ...
            return ResponseEntity.status(401)
                    .body("Impossibile stabilire una connessione al DB");
        }
    }
    ...
    public ResponseEntity<?> submitOrder(
        ...
        try (Connection conn = this.dataSource.getConnection()) {
            ...
            final String orderCode =
                    this.createTheOrderCode();

            // 1. Salviamo l'anagrafica usando i getter
            boolean infoSaved =
                    dbDelivery.insertDeliveryOrderDataInfo(
                            orderCode,
                            request.getName(),
                            request.getAddress(),
                            request.getCap(),
            ...
            if (infoSaved) {
                ...
                // 2. Convertiamo la lista di DTO
                List<Object[]> itemsForDb =
                        new ArrayList<>();
                ...
                boolean listSaved =
                        dbDelivery.insertDeliveryOrderList(
                                orderCode,
                                itemsForDb
                        );
            ...
            return ResponseEntity.status(500)
                    .body("Errore nel salvataggio dell'ordine");
